...
and 
\begin{align}
F(\{\Delta g_{i}\}_{i}) &= - \sum_{i=2}^{K} N_{i} \sum_{j=1}^{i-1} \Delta g_{j} - \sum_{\mathbf{m}}^{} C_{\mathbf{m}} \ln \frac{C_{\mathbf{m}}}{z_{\mathbf{m}}} \\
\frac{\partial F}{\partial \Delta g_{i}} &= \sum_{j=i+1}^{K} \frac{\partial F}{\partial g_{j}} \notag \\
&= \sum_{j=i+1}^{K} N_{j} (\exp(\sum_{k}^{j-1} \Delta g_{k}) \sum_{\mathbf{m}}^{} \frac{C_{\mathbf{m}} c_{j,\mathbf{m}}}{z_{\mathbf{m}}} - 1) 
\label{eq:dfddgj}
\end{align}
...
\label{eq:zdgj}
\end{equation}
and we set $g_{1}$ to zero since adding a constant to all $\{g_{i}\}_{i}$ won't change our 
answer.
