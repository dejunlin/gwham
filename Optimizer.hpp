...
#include "/home/dejun/dejun-loos-branch/Simplex.hpp"
...
template<class Kernel, 
	 class iT
        >
iT optimize_dlib_lbfgs(Kernel& funct, iT& startingpoint, const double& tol) {
...
template<class Kernel>
vector<valtype> optimize_Simplex(Kernel& funct, vector<valtype>& seeds, const double& tol, const uint& maxit)
{
  const uint ndim = seeds.size();
  Simplex<valtype> optimizer(ndim);
  optimizer.tolerance(tol);
  optimizer.maximumIterations(maxit);

  vector<valtype> ini_stepsize(ndim, 10);
  optimizer.seedLengths(ini_stepsize);

  Funct<Kernel, vector<valtype>, vector<valtype>, valtype> F(&funct);
  return optimizer.optimize(seeds, F); 
}
